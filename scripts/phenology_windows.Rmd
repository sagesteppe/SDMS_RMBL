...
# snow_free_day <- function(x){

  # this function serves to reduce the periodicity by which the weibull distribution
  ...
  gothic <- st_centroid(rmbl) %>% 
    st_transform(st_crs(x_year_snow))
  ...
  for (i in 1:length(snow_files)){
    ...
    res[i] <- data.frame('Snow' = t(extract(x_year_snow, gothic))) %>% 
      ...
      mutate(Date = str_remove(Date, 'X'),
             DOY = yday(as.Date(Date, format = "%Y.%m.%d")),
  ...
  res <- data.frame('Date' = unlist(res)) %>% 
...
input <- spp_target_ecoregions %>% 
  dplyr::select(Species, Date) %>%
  group_by(Species) %>% 
  filter(!str_detect(Date,'\\d{4}-01-\\d{2}')) %>% 
...
weibull_wrapper <- function(x){
  ...
  taxon <- x$Species[1]
  x$DOY <- lubridate::yday(as.Date(x$Date, format = "%Y-%m-%d")) + 1
  observations <- x$DOY
  sample_size <- nrow(x)
