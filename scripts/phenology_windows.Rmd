...
lwr_snw_bnd <- vector(mode = "list", length = length(snow_files))
for (i in 1:length(snow_files)){
  ...
  lwr_snw_bnd[i] <- data.frame('Snow' = t(extract(x_year_snow, gothic))) %>% 
...
lwr_snw_bnd <- data.frame('Date' = unlist(lwr_snw_bnd)) %>% 
...
upr_snw_bnd <- vector(mode = "list", length = length(snow_files))
for (i in 1:length(snow_files)){
    
  # this serves to reduce the periodicity by which the weibull distribution
  # generates biologically impossible estimates of Phenological events.
  # based on restriction of onset events via Snow cover. (See
  # Iler et al. 2021 for details.)
  ...
  x_year_snow <- brick(snow_files[i])
  upr_snw_bnd[i] <- data.frame('Snow' = t(extract(x_year_snow, gothic))) %>% 
    rownames_to_column('Date') %>% 
    mutate(Date = str_remove(Date, 'X'),
           DOY = yday(as.Date(Date, format = "%Y.%m.%d")) +1,
...
upr_snw_bnd <- data.frame('Date' = unlist(upr_snw_bnd)) %>% 
  mutate(DOY = yday(as.Date(Date, format = "%Y.%m.%d")),
         first_quart = round(quantile(DOY, 0.25), 0)) 

rm(x_year_snow,i, snow_files, rmbl, snow_crs, gothic)
```
...
cleaner_phenology_records <- spp_target_ecoregions %>% 
  ...
  filter(DOY >= lwr_snw_bnd$third_quart[1], DOY <= upr_snw_bnd$first_quart[1]) %>% # only records after snow melt and before October are kept
  arrange(DOY) %>% 
...
write.csv(weibull_estimates, paste0(here(),'/data/processed/weibull_estimates.csv'),row.names = F)

rm(as, observations, today, weibull_input, weibull_wrapper, spp_target_ecoregions)
```

```{r Species without enough records for weibull estimation, warning = F}

fixed_input <- cleaner_phenology_records %>% 
...
rm(cleaner_phenology_records, herb_wrapper, fixed_input)
```
...
flowering_est_long <- bind_rows(lapply(flowering_est_long, daily_flowers)) %>% 
  arrange(DOY) %>% 
  group_by(DOY)  %>% 
  mutate('Estimate' = if_else(Taxon %in% weibull_estimates$taxon, 'Weibull', 'Fixed'))
...
rm(weibull_estimates, fixed_estimates, lwr_snw_bnd, upr_snw_bnd)


```


Belitz MW, Larsen EA, Ries L, Guralnick RP. The accuracy of phenology estimators for use with sparsely sampled presence-only observations. Methods
Ecol Evol. 2020;11:1273–1285. https://doi.org/10.1111/2041-210X.13448
