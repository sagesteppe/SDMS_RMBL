...
We here calculate weibull based estimates of the date of flowreing onset, cessation, and the dates when 10% of individuals are flowering, peak flowering (50% of individuals), and when 9-% of individuals have flowered. 
```{r calculate weibull phenology estimates, echo = F, warning = F, eval = F}
...
cleaner_phenology_records <- spp_target_ecoregions %>% 
  ...
  filter(DOY >= lower_snow_bound$third_quart[1], DOY <= upper_snow_bound$first_quart[1]) %>% # only records after snow melt and before October are kept
  arrange(DOY) %>% 
