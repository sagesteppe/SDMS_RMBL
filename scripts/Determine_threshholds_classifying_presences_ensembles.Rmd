...
summary(allspecies_log)
confint(allspecies_log)
exp(coef(allspecies_log))
```

We can present results using a classifcation table, more importantly this table will be  used to calculate a variety of metrics for evaluating the ability of the model to accurately classify observations. 
...
ctab_train <- table(cut(train$Value, breaks = 2), cut(train$occurrence, breaks = 2))
ctab_train
...
# Concordance}
concordance <- InformationValue::Concordance(allspecies_log$y,allspecies_log$fitted.values)
conc <- concordance[["Concordance"]]
disc <- concordance[["Discordance"]]
tied <- concordance[["Tied"]]
pairs <- concordance[["Pairs"]]
...
evaluation_table <- data.frame(
  'Metric' = c('Accuracy (Training)','Accuracy (Test)','Recall',
           ...
           'Value' = c(accuracy_train, accuracy_test, Recall, TNR, 
                       ...
                       conc, disc, tied, pairs )
  )

evaluation_table$Value <- signif(evaluation_table$Value, 2)
knitr::kable(evaluation_table)

rm(accuracy_train, accuracy_test, Recall, TNR, Precision, F_Score, roc,conc, disc, tied, pairs, ctab_train, ctab_test)
```
